...
class TeamModeSkillContractTests(unittest.TestCase):
    ...
    def test_project_integration_guide_covers_install_verify_and_usage(self) -> None:
        ...
        self.assertIn("CODEX_HOME/sources/codex-team-mode", guide)
        self.assertIn("不得放入 PROJECT_ROOT 或 PROJECT_ROOT 的同级目录", guide)
        self.assertIn("macOS、Linux 和 Windows", guide)
        self.assertIn("$HOME/.codex", guide)
        self.assertIn("%USERPROFILE%\\.codex", guide)
        self.assertIn("不要手工拼接 / 或 \\", guide)
        self.assertIn("Windows PowerShell", guide)
        self.assertIn("不扫描、搜索或列出其他用户目录", guide)
        self.assertIn("本次新克隆、此前已存在，或被冲突阻塞", guide)
        self.assertIn("只创建缺失的 CODEX_HOME/sources 目录", guide)
        self.assertIn("不要求用户复制或暴露本地路径", guide)
